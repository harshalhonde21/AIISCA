...
const AllImages = () => {
  ...
  const fetchImages = async () => {
    try {
      const response = await axios.get('http://localhost:5500/api/v4/gallery/get-images');
  ...
  const handleSaveClick = async (id) => {
    ...
    try {
      await axios.put(`http://localhost:5500/api/v4/gallery/update-image/${id}`, formData, {
        headers: {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5500/api/v4/gallery/delete-image/${id}`);
